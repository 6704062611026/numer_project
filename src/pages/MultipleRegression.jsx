...
// ฟังก์ชันคำนวณ Multiple Regression
function computeMultipleRegression(points) {
  ...
  const inverse = math.inv(X);
  const coefficients = math.multiply(inverse, Y);

  return { b1: coefficients[0], b2: coefficients[1], a: coefficients[2] };
}
...
function MultipleRegression() {
  ...
  const addPoint = () => setPoints([...points, { x1: 0, x2: 0, y: 0 }]);
  ...
  // บันทึก history ลง DB
  const saveHistory = (a, b1, b2) => {
    const equation = `y = ${a.toFixed(6)} + ${b1.toFixed(6)}*x1 + ${b2.toFixed(6)}*x2`;
  ...
  const calculate = () => {
    ...
    setPredictedY(y);
    saveHistory(a, b1, b2); // save after calculation
  };
  ...
  const zGrid = x1Grid.map(x1 => x2Grid.map(x2 => predictY(a, b1, b2, x1, x2)));

  return (
    <>
      <Header3 />
      <div className="App" style={{ padding: "2rem" }}>
        <h1 style={{ color: "#1e3a8a", textAlign: "center" }}>Multiple Linear Regression (2 Variables)</h1>

        {points.map((point, index) => (
          <div key={index} style={{ textAlign: "center", marginBottom: "1rem" }}>
            <label style={{ marginRight: 8 }}>x1-{index + 1}:</label>
            <input type="number" value={point.x1} onChange={(e) => handlePointChange(index, "x1", e.target.value)} style={{ marginRight: 16 }} />
            <label style={{ marginRight: 8 }}>x2-{index + 1}:</label>
            <input type="number" value={point.x2} onChange={(e) => handlePointChange(index, "x2", e.target.value)} style={{ marginRight: 16 }} />
        ...
        <div style={{ textAlign: "center", marginBottom: "1rem" }}>
          ...
          <input type="number" value={inputX1} onChange={(e) => setInputX1(e.target.value)} style={{ marginRight: 10 }} />
          <label style={{ marginRight: 8 }}>x2 = </label>
          <input type="number" value={inputX2} onChange={(e) => setInputX2(e.target.value)} style={{ marginRight: 10 }} />
        ...
        {predictedY !== null && (
          <>
            <h2 style={{ color: "#1e3a8a", textAlign: "center", marginTop: "2rem" }}>3D Regression Surface</h2>
            <div style={{ width: 700, height: 500, margin: "0 auto" }}>
              <Plot
                data={[
                  { x: points.map(p => p.x1), y: points.map(p => p.x2), z: points.map(p => p.y), mode: "markers", type: "scatter3d", name: "Data Points", marker: { color: "red", size: 5 } },
                  { x: x1Grid, y: x2Grid, z: zGrid, type: "surface", name: "Regression Surface", opacity: 0.6, colorscale: "Blues" },
                  { x: [parseFloat(inputX1)], y: [parseFloat(inputX2)], z: [predictedY], mode: "markers", type: "scatter3d", name: "Predicted Point", marker: { color: "green", size: 6, symbol: "diamond" } },
                ]}
                layout={{ scene: { xaxis: { title: "x1" }, yaxis: { title: "x2" }, zaxis: { title: "y" } }, plot_bgcolor: "#f9fafb", paper_bgcolor: "#f9fafb", font: { color: "#1e293b" } }}
              />
            </div>
          </>
        )}
