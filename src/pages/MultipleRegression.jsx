...
class MultipleLinearRegression {
  ...
  computeCoefficients() {
    ...
    this.b1 = coefficients[0];
    this.b2 = coefficients[1];
    this.a = coefficients[2];
  }

  predict(x1, x2) {
    return this.a + this.b1 * x1 + this.b2 * x2;
  }

  getEquation() {
    return `y = ${this.a.toFixed(6)} + ${this.b1.toFixed(6)}*x1 + ${this.b2.toFixed(6)}*x2`;
  }

  getSurfaceGrid(x1Grid, x2Grid) {
    return x1Grid.map(x1 => x2Grid.map(x2 => this.predict(x1, x2)));
  }
}
...
function MultipleRegression() {
  ...
  const saveHistory = (model) => {
    const equation = model.getEquation();
  ...
  const calculate = () => {
    ...
    const model = new MultipleLinearRegression(points);
    const y = model.predict(parseFloat(inputX1), parseFloat(inputX2));
    setPredictedY(y);
    saveHistory(model);
  };

  // สร้าง regression model
  const model = new MultipleLinearRegression(points);
  ...
  const zGrid = model.getSurfaceGrid(x1Grid, x2Grid);

  return (
    <>
      <Header3 />
      <div className="App" style={{ padding: "2rem" }}>
        <h1 style={{ color: "#1e3a8a", textAlign: "center" }}>Multiple Linear Regression</h1>
